...
class User(models.AnonymousUser):
    ...
    @property
    def is_vdiuser(self):
        """Evaluates whether this user has VDI privileges.

        Returns ``True`` or ``False``.
        """
        return 'vdi' in [role['name'].lower() for role in self.roles]

    @property
    def is_domainuser(self):
        """Evaluates whether this user is a VDI domain user.

        Returns ``True`` or ``False``.
        """
        return 'domain' in [role['name'].lower() for role in self.roles]
