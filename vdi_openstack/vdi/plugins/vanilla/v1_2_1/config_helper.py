...
def get_config_value(service, name, cluster=None):
    if cluster:
        vdi_configs = generate_vdi_configs(cluster)
        if vdi_configs.get(name):
            return vdi_configs[name]
...
def generate_vdi_configs(cluster, node_group=None):
...
def generate_xml_configs(cluster, node_group, hive_mysql_passwd):
    ...
    all_cfg = generate_vdi_configs(cluster, node_group)
