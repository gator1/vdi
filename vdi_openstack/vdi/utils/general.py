...
def format_group_status(group):
    msg = "Group status has been changed: id=%s, New status=%s"
    if group:
        return msg % (group.id, group.status)
    return msg % ("Unknown", "Unknown")


def format_image_status(image):
    msg = "Image status has been changed: id=%s, New status=%s"
    if image:
        return msg % (image.id, image.status)
    return msg % ("Unknown", "Unknown")


def format_pool_status(pool):
    msg = "Pool status has been changed: id=%s, New status=%s"
    if pool:
        return msg % (pool.id, pool.status)
    return msg % ("Unknown", "Unknown")


def format_VM_status(vm):
    msg = "VM status has been changed: id=%s, New status=%s"
    if vm:
        return msg % (vm.id, vm.status)
    return msg % ("Unknown", "Unknown")
